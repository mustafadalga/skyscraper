...
import { IUpcomingBadge } from "@/_utilities/calculateUpcomingBadge";
...
export default function ProgressBar({ badge }: Props) {
    ...
    return (
        <div
            ...
            <span
                className={`${progressBar.percent} absolute z-20 ml-72 mt-1 text-xs text-white`}>%{Math.round(badge.percent)}</span>
            <BadgeIcon badgeID={badge.id}
                       className={`${progressBar.badgeIconColor} group absolute z-10 right-1.5 mt-0.5 text-sm`}/>
            <div
